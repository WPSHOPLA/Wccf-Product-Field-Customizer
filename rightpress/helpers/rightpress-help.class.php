...
final class RightPress_Help
{
    ...
    public static function get_iso_datetime($timestamp = null)
    {
        ...
        return RightPress_Help::get_adjusted_datetime($timestamp, 'Y-m-d H:i:s');
    }
    ...
    public static function get_wc_product_id($product_id = null)
    {
        ...
        // Add to cart via GET
        // WC31: Products will no longer be posts
        if (isset($_GET['add-to-cart']) && is_numeric($_GET['add-to-cart']) && RightPress_Help::post_type_is($_GET['add-to-cart'], 'product')) {
        ...
        // WC31: Products will no longer be posts
        if (isset($_POST['add-to-cart']) && is_numeric($_POST['add-to-cart']) && RightPress_Help::post_type_is($_POST['add-to-cart'], 'product')) {
        ...
        foreach ($order_items as $order_item) {
            ...
            if ($product_id = $order_item->get_product_id()) {
                ...
                if (isset($order_item['item_meta']) && is_array($order_item['item_meta'])) {
                    ...
                    // Unwrap meta
                    $item_meta = RightPress_Help::unwrap_post_meta($order_item['item_meta']);

                    // Iterate over item meta
                    foreach ($item_meta as $meta_key => $meta_value) {
                        if (RightPress_Help::string_begins_with_substring($meta_key, 'pa_')) {
    ...
    public static function get_wc_last_user_paid_order_id($user_id = null, $billing_email = null, $combine = false)
    {
        ...
        // Get paid statuses
        $paid_statuses = RightPress_Help::get_wc_order_is_paid_statuses();
