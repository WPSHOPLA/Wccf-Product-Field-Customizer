...
if (!function_exists('wccf_print_product_field')) {
     ...
     *
     * Product fields must be printed within the add to cart form,
     * otherwise field values won't be stored
     *
...
if (!function_exists('wccf_print_checkout_field')) {
     ...
     *
     * Checkout fields must be printed within the checkout form,
     * otherwise field values won't be stored
     *
...
if (!function_exists('wccf_print_user_field')) {
     ...
     *
     * User field values from custom forms are processed when action hook
     * wp_footer is called. If user fields are printed within a custom
     * registration form, user account must be created and user must be
     * authenticated in the backend before wp_footer is called.
     *
     * Only user fields set to "User Profile Field" can be used in custom forms.
     *
     * Make sure to set 'item_id' to user id if you want to display
     * existing field values for existing users on profile update.
     *
     * @param array $params key, item_id
     * @return string
     */
    ...
    function wccf_get_values($params = array())
    {
        ...
        if (!$item_id) {
            ...
            if ($context === 'product_prop') {
                $item_id = RightPress_Help::get_wc_product_id();
            }
            // Order id
            else if (in_array($context, array('checkout_field', 'order_field'), true)) {
                $item_id = RightPress_Help::get_wc_order_id();
            }
            // User id
