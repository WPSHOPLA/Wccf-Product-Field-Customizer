...
class WCCF_WC_Product
{
    ...
    public static function product_has_fields_to_display($product, $required_only = false)
    {
        ...
        if (!WCCF_WC_Product::skip_product_fields($product)) {
            ...
            if ($fields = WCCF_Product_Field_Controller::get_filtered(null, array('item_id' => $product->get_id()))) {
    ...
    {
        $product_id     = RightPress_Help::get_wc_product_id();
    ...
    public static function get_product_price($adjustment = 0, $quantity = 1)
    {
        ...
        if (get_option('woocommerce_tax_display_shop') === 'incl') {
            $price = wc_get_price_including_tax($product, array('qty' => $quantity, 'price' => $price));
        }
        else {
            $price = wc_get_price_excluding_tax($product, array('qty' => $quantity, 'price' => $price));
        }
    ...
    public static function get_ajax_request_data()
    {
        ...
        if (isset($data['variation_id']) && is_numeric($data['variation_id'])) {
            ...
            $variation_id = RightPress_Help::get_wc_variation_id_from_attributes($product_id, $attributes);
        }
    ...
    public static function print_product_property_values_in_frontend($product_id = null, $return_html = false, $skip_filter = false)
    {
        ...
        $product_id = RightPress_Help::get_wc_product_id($product_id);
    ...
    public static function prices_subject_to_adjustment()
    {
        ...
        if (self::$prices_subject_to_adjustment === null) {
            ...
            $result = get_option('wccf_prices_subject_to_adjustment');

            // No flag set
            if ($result === false) {
                $result = WCCF_Pricing::update_prices_subject_to_adjustment_flag();
            }

            // Store result in memory
            self::$prices_subject_to_adjustment = $result;
        }
    ...
    public static function get_attribute_term_ids($product)
    {
        ...
        foreach ((array) $product->get_attributes() as $attribute_key => $attribute) {
            ...
            // Get product terms
            $product_terms = (array) wc_get_product_terms($product->get_id(), $attribute_key, array('fields' => 'slugs'));
    ...
    public static function get_variation_id($variation_id = null, $check_attributes_in_request = false)
    {
        ...
        // Add To Cart
        // WC31: Products will no longer be posts
        if (!empty($_REQUEST['add-to-cart']) && is_numeric($_REQUEST['add-to-cart']) && RightPress_Help::post_type_is($_REQUEST['add-to-cart'], 'product')) {
            if (!empty($_REQUEST['variation_id']) && is_numeric($_REQUEST['variation_id']) && RightPress_Help::post_type_is($_REQUEST['variation_id'], 'product_variation')) {
        ...
        // WC31: Products will no longer be posts
        if (isset($_POST['action']) && $_POST['action'] === 'woocommerce_add_to_cart' && isset($_POST['product_id']) && is_numeric($_POST['product_id']) && RightPress_Help::post_type_is($_POST['product_id'], 'product')) {
            if (!empty($_POST['variation_id']) && is_numeric($_POST['variation_id']) && RightPress_Help::post_type_is($_POST['variation_id'], 'product_variation')) {
        ...
        if ($check_attributes_in_request) {
            ...
            if ($product_id && !empty($attributes)) {
                ...
                return RightPress_Help::get_wc_variation_id_from_attributes($product_id, $attributes);
            }
        }
    ...
    public static function get_default_variation_id($product)
    {
        ...
        if (count($all_attributes) === count($default_attributes)) {
            ...
            // Get default variation id
            return (int) RightPress_Help::get_wc_variation_id_from_attributes($product, $default_attributes);
        }
            ...
            if (!WCCF_WC_Product::skip_pricing($product, $variation)) {
                ...
                if (!WCCF_WC_Product::skip_product_fields($product, $variation) && !WCCF_WC_Product::skip_pricing($product, $variation)) {
                    ...
                    if ($has_pricing) {
                        ...
                        // Get price to adjust
                        $price = ($price_data['price'] !== null) ? $price_data['price'] : $object->get_price();
                        ...
                        // Get adjusted price
                        $adjusted_price = WCCF_Pricing::get_adjusted_price($price, $product_id, $variation_id, $wccf, $quantity, false, false);
