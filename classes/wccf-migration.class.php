...
class WCCF_Migration
{
    ...
    public function maybe_migrate_data()
    {
        ...
        if (version_compare(WCCF_VERSION, '2.0', '>=')) {
            ...
            // Redirect user
            wp_redirect(add_query_arg('wccf_migrated', RightPress_Help::get_hash()));
            exit;
        }
    }
            ...
            foreach ($old_fields as $old_field_index => $old_field) {
                ...
                if ($field->uses_options() && !empty($old_field['options']) && is_array($old_field['options'])) {
                    foreach ($old_field['options'] as $option) {
                        ...
                        $option_key = RightPress_Help::ensure_unique_string($option_key, $used_option_keys);
    ...
    public static function product_fields_in_admin_order_item_from_1($order_item_id, $order_item, $product)
    {
        ...
        if (!empty($order_item['item_meta']) && is_array($order_item['item_meta'])) {
            ...
            $unwrapped = RightPress_Help::unwrap_post_meta($order_item['item_meta']);
    ...
    public static function file_download_link_html_from_1($post_id, $field, $prepend = '', $append = '', $storage_key = 'wccf')
    {
        ...
        else if ($storage_key === 'wccf_order' && !empty($field['public']) && RightPress_Help::user_owns_wc_order(get_current_user_id(), $post_id)) {
            $access_granted = true;
        }
        // Allow access to own Checkout Fields
        else if ($storage_key === 'wccf_checkout' && RightPress_Help::user_owns_wc_order(get_current_user_id(), $post_id)) {
            $access_granted = true;
        }
        // Allow access to own Product Fields
            ...
            if (get_post_type($post_id) === 'shop_order') {
                ...
                if (RightPress_Help::user_owns_wc_order(get_current_user_id(), $post_id)) {
                    $access_granted = true;
                }
            }
            // $post_id is order item id
                ...
                // Get order id
                $order_id = RightPress_Help::get_wc_order_id_from_order_item_id($post_id);

                // Check if user owns order
                if (!empty($order_id) && RightPress_Help::user_owns_wc_order(get_current_user_id(), $order_id)) {
                    $access_granted = true;
                }
            }
        }
    ...
    public function file_download_from_1()
    {
        ...
        if (is_user_logged_in() && $_GET['wccf_version_1_file_download'] === 'wccf_checkout') {
            if (RightPress_Help::user_owns_wc_order(get_current_user_id(), $_GET['post_id'])) {
                $access_granted = true;
            }
        }
        ...
        if (is_user_logged_in() && $_GET['wccf_version_1_file_download'] === 'wccf') {
            ...
            // post_id is order item id
                ...
                // Get order id
                $order_id = RightPress_Help::get_wc_order_id_from_order_item_id($_GET['post_id']);
        ...
        if ($_GET['wccf_version_1_file_download'] === 'wccf') {
            ...
            $fields = RightPress_WC::order_item_get_meta($_GET['post_id'], '_wccf', true);

            // Also look in post meta (versions earlier than 1.2 saved it in wrong location)
            if (empty($fields)) {
                $fields = RightPress_WC::order_get_meta($_GET['post_id'], '_wccf', true);
            }
        }
        else if ($_GET['wccf_version_1_file_download'] === 'wccf_product_admin') {
            $fields = RightPress_WC::product_get_meta($_GET['post_id'], '_' . $_GET['wccf_version_1_file_download'], true);
        }
        else if (in_array($_GET['wccf_version_1_file_download'], array('wccf_order', 'wccf_checkout'), true)) {
            $fields = RightPress_WC::order_get_meta($_GET['post_id'], '_' . $_GET['wccf_version_1_file_download'], true);
        }
