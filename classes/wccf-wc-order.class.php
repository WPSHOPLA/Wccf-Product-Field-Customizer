...
class WCCF_WC_Order
{
        ...
        if ($container_open) {
            ...
            foreach ($all_fields as $field) {
                ...
                if ($stored_value !== false) {
                    ...
                    // Print field value
                    if (!RightPress_Help::is_empty($display_value)) {
                        echo $display_value;
                    }
                    else {
    ...
    public function print_order_field_values_in_frontend($order, $skip_filter = false)
    {
        ...
        $fields = WCCF_Order_Field_Controller::get_filtered(array('enabled', 'disabled', 'archived'), array('item_id' => $order->get_id()));
    ...
    public function attach_checkout_field_files($attachments, $email_id, $order)
    {
        ...
        foreach ($all_fields as $field) {
            ...
            // Get file access key
            $access_keys = (array) $field->get_stored_value($order->get_id());
    ...
    public static function get_field_html_for_backend_value_editing($field, $item_id, $quantity_index = null)
    {
        ...
        if ($context === 'user_field') {
            $stored_value = RightPress_WC::order_get_meta($item_id, $field->get_value_access_key(), true);
        }
        else {
    ...
    public function save_backend_editing_field_values($post_id, $post)
    {
        ...
        foreach ($_POST['wccf_backend_editing'] as $item_id => $field_values) {
            ...
            foreach ($field_values as $field_id => $field_value) {
                ...
                if ((RightPress_Help::is_empty($value) && $field->is_required()) || (!RightPress_Help::is_empty($value) && !WCCF_Field_Controller::validate_field_value($field, $value, $item, $quantity_index))) {
                    continue;
                }
                ...
                // WC31: maybe improve this by having a separate method and doing proper check for data integrity instead of just calling order_get_meta
                if ($field->context_is('user_field')) {
                    $stored_value = RightPress_WC::order_get_meta($item_id, $field->get_value_access_key($quantity_index), true);
                }
                else {
                ...
                // Store new value
                if ($field->context_is('user_field')) {
                    RightPress_WC::order_update_meta_data($item_id, $field->get_value_access_key(), $value);
                }
                else {
                ...
                if ($field->has_options()) {
                    ...
                    // WC31: maybe improve this by having a separate method and doing proper check for data integrity instead of just calling order_get_meta
                    if ($field->context_is('user_field')) {
                        $extra_data = RightPress_WC::order_get_meta($item_id, $field->get_extra_data_access_key(), true);
                    }
                    else {
                    ...
                    // Store extra data
                    if ($field->context_is('user_field')) {
                        RightPress_WC::order_update_meta_data($item_id, $field->get_extra_data_access_key(), $extra_data);
                    }
                    else {
    ...
    public function print_field_values_backend_order_view($order)
    {
        ...
        foreach ($values as $field_id => $value) {
            ...
            // Allow field value editing
            if (WCCF_Field_Controller::field_value_editing_allowed('user_field', $field, $value, $order->get_id())) {
                echo '<span class="wccf_backend_editing_value" data-wccf-backend-editing="1" data-wccf-field-id="' . $field->get_id() . '" data-wccf-item-id="' . $order->get_id() . '">';
                $backend_value_editing_allowed = true;
            }
    ...
    public static function get_field_values_from_order($order, $fields, $context = null)
    {
        ...
        foreach ($fields as $field) {
            ...
            if ($context === 'user_field') {
                $field_id_from_meta = RightPress_WC::order_get_meta($order, $field->get_id_access_key(), true);
            }
            else {
                $field_id_from_meta = $field->get_data($order, $field->get_id_access_key(), true);
            }
            ...
            if ($context === 'user_field' && !RightPress_WC::order_meta_exists($order, $field->get_value_access_key())) {
                continue;
            }
            else if ($context !== 'user_field' && !$field->data_exists($order, $field->get_value_access_key())) {
                continue;
            }

            // Get field stored value
            if ($context === 'user_field') {
                $stored_value = RightPress_WC::order_get_meta($order, $field->get_value_access_key(), true);
            }
            else {
                $stored_value = $field->get_data($order, $field->get_value_access_key(), true);
            }
    ...
    public function print_user_field_values_in_frontend_order_view($order, $skip_filter = false)
    {
        ...
        // Allow developers to skip displaying frontend field values in default position
        if (!$skip_filter && !apply_filters('wccf_frontend_display_user_field_values', true, $display, $order->get_id())) {
            return;
        }
    ...
    public function add_user_field_values_for_emails($fields, $sent_to_admin, $order)
    {
        ...
        if (!apply_filters('wccf_email_display_user_field_values', true, $display, $order->get_id())) {
            return $fields;
        }
