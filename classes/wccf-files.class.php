...
class WCCF_Files
{
    ...
    public static function get_unique_file_key($key_type, $upload_directory = null, $current_key = null, $files = array())
    {
        ...
        while (!$key && $attempts) {
            ...
            if (!isset($random) && $current_key !== null) {
                ...
                $random = RightPress_Help::get_hash(true);
            }
            ...
            // Validate access key
            if ($key_type === 'access' && !RightPress_Help::meta_key_exists('_wccf_file_' . $random) && !isset($files[$random])) {
                $key = $random;
                break;
            }
    ...
    public static function get_data_by_access_key($access_key, $item_id = null)
    {
        ...
        $file_data = RightPress_Help::get_meta(WCCF_Field::get_file_data_access_key($access_key), null, $item_id);
    ...
    public static function get_temporary_file($file_data)
    {
        ...
        $temp_directory_path = WCCF_Files::get_temp_directory(RightPress_Help::get_hash());
    ...
    public static function current_user_can_access($access_key, $redirect_to_login = true)
    {
        ...
        $extended_file_data = RightPress_Help::get_meta_row($data_access_key);
        ...
        if (!$user_id) {
            ...
            if ($redirect_to_login) {
                $current_url = RightPress_Help::get_request_url();
        ...
        // Product Field
        if ($field->context_is('product_field') && RightPress_Help::user_owns_wc_order_item($user_id, $item_id)) {
            return true;
        }

        // Checkout Field
        if ($field->context_is('checkout_field') && RightPress_Help::user_owns_wc_order($user_id, $item_id)) {
            return true;
        }

        // Order Field
        if ($field->context_is('order_field') && RightPress_Help::user_owns_wc_order($user_id, $item_id) && $field->is_public()) {
            return true;
        }
        ...
        try {
            ...
            if (!empty($_POST['item_id'])) {
                ...
                if ($field->context_is('product_field')) {
                    RightPress_WC::order_update_meta_data($_POST['item_id'], $meta_access_key, $file_data);
                }
                else {
            ...
            else if ($session = WCCF_WC_Session::initialize_session()) {
            ...
            echo json_encode(array(
                ...
                'file_size'     => $values[$field_id_for_name]['value']['size'],
            ));
            exit;
        }
        // Error occurred
