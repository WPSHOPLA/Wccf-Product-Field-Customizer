...
class WCCF_WP_User
{
    ...
    public function __construct()
    {
        ...
        // Save user field values from custom forms
        if (!is_admin() && !empty($_POST['wccf']['user_field'])) {
            add_action('wp_footer', array($this, 'save_user_field_values_custom_form'));
        }
    ...
    public function save_user_field_values_create($user_id)
    {
        ...
        // Store field values
        $user_field_type = ((is_admin() & !is_ajax()) ? null : 'user_profile');
        WCCF_Field_Controller::store_field_values($item, 'user_field', true, false, $user_field_type);
     ...
     * @return void
     */
    public function save_user_field_values_custom_form()
    {
