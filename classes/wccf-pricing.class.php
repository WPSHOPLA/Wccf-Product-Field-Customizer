...
class WCCF_Pricing
{
     ...
     * @return float
     */
    public static function get_adjusted_price($price, $product_id, $variation_id = null, $posted = array(), $quantity = 1, $calculate_taxes = false, $rounded = true, $product = null, $is_cart = false)
    {
        ...
        $adjusted_price = self::get_price_adjusted_by_fields($adjusted_price, $product_id, $variation_id, 'product_prop', array(), 1, false, $rounded, $product, $is_cart);
        ...
        if (!WCCF_WC_Product::skip_product_fields($product_id, $variation_id)) {
            $adjusted_price = self::get_price_adjusted_by_fields($adjusted_price, $product_id, $variation_id, 'product_field', $posted, $quantity, $calculate_taxes, $rounded, $product, $is_cart);
        }
     ...
     * @return float
     */
    private static function get_price_adjusted_by_fields($price, $product_id, $variation_id, $context, $posted = array(), $quantity = 1, $calculate_taxes = false, $rounded = true, $product = null, $is_cart = false)
    {
        ...
        for ($i = 0; $i < $quantity; $i++) {
            ...
            foreach ($fields as $field) {
                ...
                if ($context === 'product_field') {
                    $value = self::get_product_field_value_for_price_adjustment($field, $product_id, $product, $posted, $i, $is_cart);
                }
     ...
     * @return mixed
     */
    private static function get_product_field_value_for_price_adjustment($field, $product_id, $product = null, $posted = array(), $quantity_index = null, $is_cart = false)
    {
        // Check if any values were posted
        if (!empty($posted)) {

            // Ignore quantity index in cart
            if ($is_cart) {
                $quantity_index = null;
            }
    ...
    public static function adjust_price($original_price, $price, $product_id, $pricing_method, $pricing_value, $field_value, $calculate_taxes)
    {
        ...
        if ($calculate_taxes) {
            $tax_adjusted_pricing_value = wc_get_price_to_display($product, array('price' => $pricing_value));
        }
        else {
    ...
    public static function get_adjustment_value($original_price, $price_to_adjust, $pricing_method, $pricing_value, $field_value)
    {
        ...
        switch ($pricing_method) {
            ...
            case 'advanced_fees_fee_x_value':
                $adjustment_value = ($pricing_value * abs((float) RightPress_Help::normalize_string_decimal($field_value)));
                break;
    ...
    public static function get_checkout_fees($cart_total, $fields = array(), $posted = array())
    {
        ...
        foreach ($fields as $field) {
            ...
            // Field does not have options but has non-empty value
            else if (!RightPress_Help::is_empty($value)) {
    ...
    public static function process_amount_manipulations($amount)
    {
        ...
        $amount = RightPress_Help::get_amount_in_currency($amount);
    ...
    /**
     * Update prices subject to adjustment flag in the database
     *
     * @access public
     * @return bool
     */
    public static function update_prices_subject_to_adjustment_flag()
    {
        // Run query to find product fields or product properties with pricing values
        ...
        // Check if at least one field has pricing value set
        $result = (((int) $query->found_posts) > 0) ? 1 : 0;

        // Update option
        update_option('wccf_prices_subject_to_adjustment', $result, false);
