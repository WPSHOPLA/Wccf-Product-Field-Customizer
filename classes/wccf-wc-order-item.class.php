...
class WCCF_WC_Order_Item
{
    ...
    public function __construct()
    {
        ...
        if (RightPress_Help::wc_version_gte('3.0.4')) {
    ...
    public function get_formatted_meta_data($formatted_meta, $order_item)
    {
        ...
        // Allow developers to skip displaying product field values in emails
        if (doing_action('woocommerce_email_order_details') && !apply_filters('wccf_email_display_product_field_values', true, $display_values, $order_item->get_id())) {
            return $formatted_meta;
        }
    ...
    public function format_order_item_meta($formatted_meta, $item_meta)
    {
        ...
        $unprocessed_meta = RightPress_Help::unwrap_post_meta($item_meta->meta);
    ...
    public function display_backend_order_item_meta($order_item_id, $order_item, $product)
    {
        ...
        $unprocessed_meta = RightPress_Help::unwrap_post_meta(get_metadata('order_item', $order_item_id));
    ...
    public function get_fields_from_order_item_meta($unprocessed_meta, $product)
    {
        ...
        foreach ($unprocessed_meta as $meta_key => $meta_value) {
            ...
            if (!$field || RightPress_Help::post_is_trashed($field->get_id())) {
                continue;
            }
    ...
    public function get_display_values_from_order_item_meta_legacy($unprocessed_meta, $product, $fields = null, $order_item_id = null)
    {
        ...
        foreach ($fields as $field) {
            ...
            // Iterate over values
            foreach ($unprocessed_meta as $meta_key => $meta_value) {
                if (RightPress_Help::string_contains_phrase($meta_key, $field->get_value_access_key())) {
                    ...
                    if ($meta_key !== $field->get_value_access_key()) {
                        ...
                        $extra_data = RightPress_Help::array_value_or_false($unprocessed_meta, $field->get_extra_data_access_key($quantity_index));
                    ...
                    $field_value = RightPress_Help::array_value_or_false($unprocessed_meta, $field->get_value_access_key($quantity_index));
                    ...
                    if ($field_value !== false) {
                        ...
                        $field_value = array(
                            'value' => $field_value,
                            'data'  => RightPress_Help::array_value_or_false($unprocessed_meta, $field->get_extra_data_access_key($quantity_index)),
                        );
