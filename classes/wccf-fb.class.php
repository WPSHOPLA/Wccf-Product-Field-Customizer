...
class WCCF_FB
{
        ...
        if (!empty($params['options']) && is_array($params['options'])) {
            ...
            foreach ($params['options'] as $group_key => $group) {
                ...
                foreach ($group['options'] as $option_key => $option) {
                    ...
                    if (WCCF_FB::is_backend() && RightPress_Help::string_contains_phrase($params['id'], 'wccf_post_config_conditions_other_field_id_')) {
    ...
    {
        // Value must be date
        if (!RightPress_Help::is_date($value, WCCF_Settings::get('date_format', true))) {
